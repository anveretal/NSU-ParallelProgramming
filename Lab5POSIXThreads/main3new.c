...
// когда мы получаем task (и выходим из lock), и пока выполняется задача, другой процесс может
// запросить у этого половину задач, и так, которая выполняется сейчас тоже отправится (т.к. size)
// меняется после execute_task
...
void generate_tasks(int iteration) {
    ...
    for (int i = 0; i < 50 * rank + 50; i++) {
        Task task;
        //task.repeat_count = 10000 * (abs(rank - (iteration % size)) + 100000);
        //task.repeat_count = 1000000 * (rank) + 1000000;
        task.repeat_count = 1000000;
        //task.repeat_count = 10000;
        //task.repeat_count = abs(50 - i % 100) * abs(rank - (iteration % size)) * 2048;
