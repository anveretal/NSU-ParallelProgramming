...
#define REQUEST_DELAY_US 100
#define LOAD_BALANCE_THRESHOLD 0.3
...
atomic_llong expected_weight = 0;
atomic_llong actual_weight = 0;
...
void* worker_thread(void* arg) {
    while (!shared.is_done) {
        ...
        if (get_task(&local_tasks, &task)) {
            ...
            // Более агрессивная стратегия запроса задач
            int req = 1;
...
void* request_handler_thread(void* arg) {
    while (!shared.is_done) {
        ...
        if (flag) {
            ...
            pthread_mutex_lock(&local_tasks.lock);
            int available = local_tasks.size;
            int can_share = available > TASKS_COUNT * LOAD_BALANCE_THRESHOLD;
            
            if (can_share) {
                int tasks_to_send = available / 2;
...
int main(int argc, char** argv) {
    ...
    for (int iter = 1; iter <= 10; iter++) {
        ...
        do {
            ...
            pthread_mutex_lock(&local_tasks.lock);
            local_done = (local_tasks.size == 0) && (shared.tasks_processed >= TASKS_COUNT);
            pthread_mutex_unlock(&local_tasks.lock);
